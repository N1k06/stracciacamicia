// search_full_20.cu
//
// Ricerca ESAUSTIVA su un mazzo ridotto a 20 carte (proporzione 14:2:2:2,
// identica al 28:4:4:4 del mazzo reale), come test end-to-end della pipeline
// completa (unranking + simulate + kernel persistente con coda di lavoro
// atomica, esattamente come descritto nel README) prima di passare al mazzo
// reale da 40 carte. A differenza del run di produzione, qui possiamo
// permetterci di coprire l'INTERO spazio (~3.488.400 configurazioni) in un
// singolo lancio di kernel, senza bisogno di dividerlo in batch.
//
// Riporta:
//   - le configurazioni che raggiungono il tetto di turni (candidate
//     "potenzialmente infinite", stesso criterio del progetto principale)
//   - statistiche riassuntive (turno massimo trovato, istogramma dei turni)
//
// Uso:
//   nvcc -O3 -arch=sm_XX search_full_20.cu -o search_full_20
//   ./search_full_20 table_20.bin [max_turns=5000] [hit_threshold=5000] [max_hits=100000] [out=hits_20.bin]
...
// Stato globale condiviso tra tutti i thread (kernel persistente, §10.2 README)
...
__device__ __forceinline__ int bucket_for(int turns, int max_turns) {
    if (turns >= max_turns) return 5;  // ha raggiunto il tetto: candidato
...
// Kernel persistente: ogni thread resta in loop pescando il prossimo rank
// da un contatore atomico condiviso, invece di ricevere un rank fisso. Questo
// bilancia il carico anche se alcune partite durano molto piu' a lungo di
// altre (evita che un intero warp resti bloccato sul thread piu' lento).
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Uso: %s table_20.bin [max_turns=5000] [hit_threshold=5000] [max_hits=100000] [out=hits_20.bin]\n", argv[0]);
        return 1;
    }
    ...
    const char *out_path = argc > 5 ? argv[5] : "hits_20.bin";
    ...
    // Il totale delle configurazioni e' semplicemente multinomial(counts) = tbl(counts)
    ...
    if (h_hit_count > max_hits) {
        fprintf(stderr, "\nATTENZIONE: buffer hit saturo (%u trovati, capacita' %u). Aumenta max_hits e rilancia.\n",
                h_hit_count, max_hits);
    }
