...
# Validazione della versione bit-packed (prima del mazzo da 40 carte)

Prima di usare la rappresentazione bit-packed (necessaria per le performance
a piena scala, README §8) nel run di produzione, la validiamo riutilizzando
gli stessi identici risultati già ottenuti e ispezionati manualmente sul
mazzo da 20 carte: se la conversione è corretta, la versione bit-packed deve
trovare esattamente gli stessi 16 candidati, con esattamente gli stessi
turni.

## 1. Carica i file

- `search_full_20_bitpacked.cu`
- `compare_bitpacked_hits.py` (contiene già incorporati i 16 candidati noti
  del tuo run precedente — non serve ricaricare `hits_validation_report.txt`)

## 2. Compila ed esegui

```
!nvcc -O3 -arch=sm_75 search_full_20_bitpacked.cu -o search_full_20_bitpacked
!./search_full_20_bitpacked table_20.bin
```

(riusa `table_20.bin` già generato nei passi precedenti)

## 3. Confronta con i risultati già validati
...
Output atteso:
```
Candidati attesi (versione ad array, gia' validata): 16
Candidati trovati dalla versione bit-packed: 16
max_turns usato nella ricerca bit-packed: 5000

TUTTI I 16 CANDIDATI COINCIDONO ESATTAMENTE tra le due versioni.
La conversione a bit-packed e' validata: stesso insieme di rank, stessi turni.
```

Se questo passa, hai una conferma end-to-end molto solida: stessa logica,
stesso risultato, rappresentazione dati diversa e più efficiente. **Solo a
questo punto** procedi al run di produzione.

---

# Run di produzione: mazzo reale da 40 carte

## ⚠️ Prima di iniziare: monta Google Drive

Il run di produzione dura giorni o settimane, molto più a lungo di una
singola sessione Colab. **Checkpoint e file degli hit vanno salvati su Google
Drive**, altrimenti si perdono ogni volta che la sessione si disconnette:
...
Poi crea una cartella dedicata, ad esempio:
```
!mkdir -p /content/drive/MyDrive/straccia_camicia
```

Userai percorsi tipo `/content/drive/MyDrive/straccia_camicia/checkpoint.txt`
per `checkpoint_file` e `hits_file` nei comandi seguenti.

## 1. Carica i file

- `straccia_search_40.cu`
- `multinomial_table.bin` (quello già generato con `gen_table_and_tests.py`
  nel primo passo — stessa composizione 28/4/4/4, non serve rigenerarlo)
- `straccia_common.py` e `inspect_hits_40.py` (per ispezionare i risultati
...
Controlla l'output di `--ptxas-options=-v`: non dovrebbero comparire `spill
stores`/`spill loads` diversi da zero (README §8.6). Se compaiono, è un
segnale che la conversione bit-packed non sta tenendo tutto in registri come
previsto — vale la pena indagare prima di lanciare il run completo.

## 3. Taratura preliminare (facoltativa ma consigliata)

Prima del run completo, prova un batch piccolo per stimare il throughput
reale sulla GPU che ti ha assegnato Colab:

```
!./straccia_search_40 multinomial_table.bin 10000000 5000 100000 60 \
    /content/drive/MyDrive/straccia_camicia/checkpoint_test.txt \
    /content/drive/MyDrive/straccia_camicia/hits_test.bin
```

Questo elabora un batch da 10 milioni di configurazioni con un budget di 60
secondi, dandoti throughput reale (`M/s` nell'output) da cui stimare il tempo
totale per l'intero spazio (~1,9358 × 10¹⁴ configurazioni). Cancella
`checkpoint_test.txt`/`hits_test.bin` dopo, non sono il run vero.

## 4. Lancia il run di produzione

```
!./straccia_search_40 multinomial_table.bin 200000000 5000 100000 36000 \
    /content/drive/MyDrive/straccia_camicia/checkpoint.txt \
    /content/drive/MyDrive/straccia_camicia/hits_40.bin
```

Parametri (tutti opzionali, in quest'ordine):
1. `multinomial_table.bin` — la tabella (obbligatorio)
2. `batch_size` — default 200.000.000 (2×10⁸, come nello script Python originale)
3. `max_turns` — default 5000 (il tetto = anche la soglia di "candidato", coincidono per definizione nel progetto principale)
4. `max_hits_per_batch` — default 100.000 (capacità del buffer hit per singolo batch)
5. `time_budget_seconds` — default 36000 (10 ore, prudente rispetto al limite di ~12h di Colab)
6. `checkpoint_file` — dove salvare il progresso (**puntalo a Drive**)
7. `hits_file` — dove accumulare i candidati trovati (**puntalo a Drive**)

Il programma stampa una riga per ogni batch completato:
```
batch_start=0  size=200000000  tempo=X.XXs  throughput=XX.XM/s  hit=N  progresso=0.000103%
```

e si ferma da solo dopo ~10 ore, salvando il checkpoint. **Rilancia lo stesso
identico comando** per riprendere esattamente da dove si era fermato — questo
è il flusso di lavoro normale su più sessioni Colab, non un'eccezione.

## 5. Ispeziona i risultati (in qualsiasi momento, anche a metà run)

```
!python3 inspect_hits_40.py /content/drive/MyDrive/straccia_camicia/hits_40.bin \
    /content/drive/MyDrive/straccia_camicia/hits_40_report.txt
```

Ricostruisce la sequenza di carte per ogni candidato e riverifica il
conteggio turni in Python, esattamente come fatto per il mazzo da 20 carte.
Puoi eseguirlo in qualsiasi momento, anche mentre il run principale è ancora
in corso in un'altra sessione — legge semplicemente lo stato attuale di
`hits_40.bin`.

## Cosa monitorare durante il run

- **Throughput (`M/s`)**: se cala progressivamente nel tempo su GPU dello
  stesso tipo, potrebbe indicare throttling termico lato Colab (fuori dal tuo
  controllo) o un batch con partite insolitamente lunghe.
- **Buffer saturo** (`ATTENZIONE: buffer saturo...`): se compare, quel
  batch specifico ha superato `max_hits_per_batch` candidati — aumenta il
  valore e rilancia manualmente solo quel batch (nota il `batch_start`
  stampato) se vuoi recuperarli tutti; altrimenti quelli oltre il limite sono
  semplicemente andati persi per quel batch (ma il checkpoint è comunque
  avanzato correttamente).
- **Progresso** (`progresso=X%`): a `batch_size=2×10⁸`, un run completo
  richiede circa 967.922 batch. Puoi stimare il tempo rimanente dal
  throughput osservato nei log.